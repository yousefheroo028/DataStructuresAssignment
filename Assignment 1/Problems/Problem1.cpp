...
public:
	...
	Guest(const string& name, const string& contact, const string& iftar_date) : name(name), contact(contact),
...
public:
	...
	explicit IftarManager(const int& no_guests) : no_guests(no_guests)
	{
	...
	{
		const auto temp = new Guest[no_guests + 1];
